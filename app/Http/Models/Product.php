...
class Product extends Model
{
    ...
    protected $appends  = ['text_preorder','text_min_order'];

    public function getTextPreorderAttribute()
    {
        if($this->status_preorder == 1){
            $text = "PO ".$this->value_preorder.' Hari';
        }else{
            $text = "PO ".$this->value_preorder.' Jam';
        }
        return $text;
    }
    
    public function getTextMinOrderAttribute()
    {
      
        $text = "Min Order ".$this->min_transaction;
        return $text;
    }
    ...
    public function multiple_photo()
    {
        return $this->hasMany(\App\Http\Models\ProductMultiplePhoto::class, 'id_product');
    }
