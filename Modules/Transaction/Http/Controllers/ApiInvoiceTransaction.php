...
class ApiInvoiceTransaction extends Controller
{
    public $saveImage = "img/transaction/manual-payment/";
    public $savePDF = "pdf/transaction/";
    ...
    {
        $setting['data'] = $this->callTransactionDetail($id);
        if(isset($result['status'])&&$result['status']=='fail'){
            return false;
        }
        $setting['logo_its'] = base64_encode(Storage::get(Setting::where('key', 'logo_its')->first()['value']));
        $setting['ttd_finance'] = base64_encode(Storage::get(Setting::where('key', 'ttd_finance')->first()['value']));
        ...
        $png = QrCode::format('png')->size(150)->generate($setting['data']['qrcode']);
        $setting['qrcode'] = base64_encode($png);     
        $path = $this->savePDF.$setting['data']['transaction_receipt_number'].'.pdf';
        Storage::disk(env('STORAGE'))->delete($path);
        $pdf = PDF::loadview('pdf.pdf',$setting);
        $data = Storage::put($path, $pdf->output(),'public');
        if($data){
            Transaction::where(['id_transaction' => $id])->update([
                'file_invoice'=>$path
            ]);
        }
        return view('pdf.pdf', $setting);
    }

    public function callTransactionDetail($id)
    {
       ...
       $transactionProducts = TransactionProduct::join('products', 'products.id_product', 'transaction_products.id_product')
                            ...
                            ->select('transaction_products.*', 'products.product_name','products.product_code','products.min_transaction')->get()->toArray();
        ...
        $group = TransactionGroup::where('id_transaction_group', $transaction['id_transaction_group'])->with(['department'])->first();
      ...
      $call = User::where('id',$merchant['id'])->first();
        $result = [
            ...
            'name_department' => $group['department']['name_department']??null,
            ...
            'date_order_received' => MyHelper::dateFormatInd(date('Y-m-d H:i', strtotime($transaction['date_order_received'])), true),
            ...
            'address' => $address,
            'transaction_shipment_text' => 'Rp ' . number_format((int)$transaction['transaction_shipment'], 0, ",", "."),
            'transaction_subtotal' => 'Rp ' . number_format((int)$transaction['transaction_subtotal'], 0, ",", "."),
            'transaction_discount' => 'Rp ' . number_format((int)$transaction['transaction_discount'], 0, ",", "."),
            'transaction_biaya_lain' => 'Rp ' . number_format((int)$transaction['transaction_tax']+(int)$transaction['transaction_service'], 0, ",", "."),
