...
class ApiProductController extends Controller
{
    ...
    public function checkInputProduct($post = [], $type = null)
    {
        ...
        if (isset($post['status_preorder'])) {
            $data['status_preorder'] = (int)$post['status_preorder'];
        }
        if (isset($post['value_preorder'])) {
            $data['value_preorder'] = (int)$post['value_preorder'];
        }
        ...
        if ($save) {
            ...
            if (isset($post['photo'])) {
                ...
                $delete = $this->deletePhoto($post['id_product_photo']);
                    //create photo
                    $upload = MyHelper::uploadPhotoAllSize($post['photo'], 'img/product/');

                if (isset($upload['status']) && $upload['status'] == "success") {
                    $cek = ProductPhoto::where('id_product', $post['id_product'])->where('id_product_photo', $post['id_product_photo'])->first();
                    if($cek){
    ...
    public function detail(Request $request)
    {
        ...
        if ($productGlobalPrice) {
            ...
            if($productGlobalPrices){
                 ...
                 $product['product_price'] = (int)$productGlobalPrices['product_price'];
            }else{
        ...
        if($cart){
            if($cart->qty >=  $product['min_transaction']){ 
             $product['qty'] = $cart->qty?? $product['min_transaction'];
            }
        }
